...
if __name__ == '__main__':
    ...
    if args.help:
        print("####################################################################################################")
        print("# DeepTCRpred v1.0: a sequence-based predictor of TCR-pMHC interaction")
        print("####################################################################################################")
        print("Usage: python DeepTCRpred.py --model [DeepTCRpred_model_name] --input [input_file_of_TCRs] --output [output_file]")
        print("e.g.: python DeepTCRpred.py --model A0201_GILGFVFTL --input ./test/test.csv --output ./test/output.csv")
        print("----------------------------------------------------------------------------------------------------")
        print("Arguments:")
        print("\t[-m] or [--model]. The name of the DeepTCRpred model. 146 pre-trained DeepTCRpred models are currently available" )
        ...
        print("\t[--list_models]. To visualize the 146 DeepTCRpred models for which we can run predictions. Models with less than 50 training TCRs have low confidence")
        print("\t[--batch_size]. The default batch size is 1. If you have a large dataset of TCRs to test, increasing the batch_size can speed DeepTCRpred up")
        print("\t[--download]. To download a specific pretrained DeepTCRpred model (E.g. python DeepTCRpred.py A0201_GILGFVFTL to download the corresponding pretrained DeepTCRpred model)")
        print("\t[--download_all]. To download the 146 pretrained DeepTCRpred models")
        print("\t[--download_high]. To download the 43 high-confidence pretrained DeepTCRpred models (with more than 50 training TCRs)")
    ...
    if args.list_models:
        ...
        s = "####################################################################################################" + '\n' \
             + "# DeepTCRpred: a sequence-based predictor of TCR-pMHC interaction" + '\n' \
            "####################################################################################################" + '\n' \
            "Usage: python DeepTCRpred.py --model [DeepTCRpred_model_name] --input [input_file_of_TCRs] --output [output_file]" + '\n' \
            "e.g.: python DeepTCRpred.py --model A0201_GILGFVFTL --input ./test/test.csv --output ./test/output.csv" + '\n' \
            "" + '\n' \
            "146 pre-trained DeepTCRpred models are available. Models with less than 50 training TCRs have low confidence." + '\n' \
            "Use the arrow keys to scroll the list of DeepTCRpred models and press \'q\' to exit" + '\n' \
    ...
    if (args.download_all) | (args.download_high):
        ...
        for model_name in df['DeepTCRpred_model_name'].values:
            if os.path.exists( os.path.join(path_pretrained_models, "model_{0}.ckpt".format(model_name))):
                print("{0} DeepTCRpred model already downloaded".format(model_name))
            else:
                url = "https://zenodo.org/record/7930623/files/model_"+model_name+".ckpt"
                print("Downloading DeepTCRpred model for {0}".format(model_name))
    ...
    if args.download != None:
        ...
        if os.path.exists( os.path.join(path_pretrained_models, "model_{0}.ckpt".format(args.download))):
            print("{0} DeepTCRpred model already downloaded".format(args.download))
        else:
            filename = wget.download(url, out = path_pretrained_models)
        sys.exit(0)
    ...
    ##to make input compatible with DeepTCRpred format
    args.train = None
    ...
    if (args.path_checkpoint == None) or (os.path.exists(args.path_checkpoint) == False):
        print("*** Error! Model not loaded. Is {0} a valid DeepTCRpred model name? For help, run ./DeepTCRpred.py --help ***".format(args.model))
        sys.exit(0)
    ...
    model_name = df_epi_info['DeepTCRpred_model_name'].values[0]
    ...
    if args.test != None:
        ...
        print("#########################################################################")
        print("###### DeepTCRpred: a sequence-based predictor of TCR-pMHC interaction  ####")
        print("#########################################################################")
        print("DeepTCRpred model: {0} ".format(args.model))
        ...
        f.write("#\n")
        f.write("# To cite DeepTCRpred,  please refer to: XXXXX\n")
        f.write("#---------------------------------------------------------------------------\n")
        f.write("# Binding predictions with DeepTCRpred model {0}\n".format(model_name))
        if good_model:
            f.write("# Number of training data {0}. Internal AUC {1:.2f}\n".format(n_seq, auc_internal))
        else:
            f.write("# Number of training data {0} (<50!). Low-confidence DeepTCRpred model! \n".format(n_seq))
        f.write("#---------------------------------------------------------------------------\n")
        f.write("##############################################################################################\n")
        f.close()
